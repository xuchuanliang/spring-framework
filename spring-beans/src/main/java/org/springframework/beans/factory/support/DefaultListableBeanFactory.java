...
public class DefaultListableBeanFactory extends AbstractAutowireCapableBeanFactory
	...
	public void preInstantiateSingletons() throws BeansException {
		...
		//beanDefinitionNames保存了所有的bean名称
		List<String> beanNames = new ArrayList<>(this.beanDefinitionNames);

		// Trigger initialization of all non-lazy singleton beans...
		//触发实例化所有的non-lazy 单例bean
		//下面的循环会触发所有非懒加载的single bean初始化工作
		for (String beanName : beanNames) {
			//合并父 Bean 中的配置，注意 <bean id="" class="" parent="" /> 中的 parent
			RootBeanDefinition bd = getMergedLocalBeanDefinition(beanName);
			//判断非抽象、单例、非懒加载的beanDefinition
			if (!bd.isAbstract() && bd.isSingleton() && !bd.isLazyInit()) {//(不是抽象类 && 是单例 && 不是懒加载)
				//是否是FactoryBean
				if (isFactoryBean(beanName)) {
					//如果是FactoryBean，在beanName之前增加&符号再调用doGetBean方法
					Object bean = getBean(FACTORY_BEAN_PREFIX + beanName);
					//如果bean是FactoryBean类型
					if (bean instanceof FactoryBean) {
						FactoryBean<?> factory = (FactoryBean<?>) bean;
						boolean isEagerInit;
						//判断当前FactoryBean是否是SmartFactoryBean类型
		...
		//到此处所有的单例、非lazy的bean已经初始化完成，
		//如果我们的bean实现了SmartInitializingSingleton接口，则会在此处被回调
		for (String beanName : beanNames) {
	...
	public void registerBeanDefinition(String beanName, BeanDefinition beanDefinition)
		...
		if (existingDefinition != null) {
			//如果已经存在了相同beanName的bean
			//判断bean是否允许被覆盖：allowBeanDefinitionOverriding属性
